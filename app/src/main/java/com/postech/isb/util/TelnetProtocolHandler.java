...
public abstract class TelnetProtocolHandler {
  ...
  /** Telnet option: sga */
  private final static byte TELOPT_SGA   = (byte)3;  /* suppress go ahead */
  ...
  /* Added by Jeongbong Seo */
  /** Telnet option: Terminal Speed */
  private final static byte TELOPT_TSPEED  = (byte)32;  /* terminal type */
  /** Telnet option: TX Display Location */
  private final static byte TELOPT_XDISPLAY  = (byte)35;  /* terminal type */
  /** Telnet option: NEw Environment Option */
  private final static byte TELOPT_NENV  = (byte)39;  /* terminal type */  
  /** Telnet option: Remote Flow Control */
  private final static byte TELOPT_REMOTE_FLOW_CNT = (byte)33;  /* terminal type */
  /* Until Here */
  ...
  private void handle_sb(byte type, byte[] sbdata) throws IOException {
	  if(debug > 1)
		  System.err.print("TelnetIO.handle_sb("+type+") : ");
	  
	  switch (type) {
	  case TELOPT_TTYPE:
		  if(debug > 1)
			  System.err.println("Terminal Type.\n");
		  ...
		  if (sbdata.length>0 && sbdata[0]==TELQUAL_SEND) {
			  ...
			  int length = ttype.length() + 6;
			  byte [] tmpBuffer = new byte[length];

			  System.arraycopy(IACSB,0,tmpBuffer,0,2);
			  tmpBuffer[2] = TELOPT_TTYPE;
			  tmpBuffer[3] = TELQUAL_IS;			  
			  System.arraycopy(ttype.getBytes(),0,tmpBuffer,4,ttype.length());
			  System.arraycopy(IACSE,0,tmpBuffer,length-2,2);
...
*/
   ...
   */
  public int negotiate(byte nbuf[], int offset) throws IOException {
	  ...
	  while(!dobreak && (boffset < count) && (noffset < nbuf.length)) {
		  ...
		  if(debug > 3) {
			  Byte B = new Byte(b);
			  System.err.print("byte: " + b + "\n");
		  }
		  switch (neg_state) {
		  
		  case STATE_DATA:
			  if (b==IAC) {
				  if(debug > 2) System.err.print("Command Start\n");
				  neg_state = STATE_IAC;
		  ...
		  case STATE_IAC:
			  ...
			  case EOR:
				  ...
				  break;
			  case SB:
				  if(debug > 2) System.err.print("Suboption Begin ");
		  ...
		  case STATE_IACWILL:
			  ...
			  case TELOPT_BINARY:
				  ...
				  reply = DO;
				  break;
			  case TELOPT_REMOTE_FLOW_CNT:
				  if(debug > 2) System.err.println("RemoteFlowControl");
				  reply = DONT;
				  break;
			  default:
		  ...
		  case STATE_IACDO:
			  ...
			  case TELOPT_NAWS: // Window Size
				  ...
				  receivedDX[b] = DO;
				  if(size == null) {
					  if(debug > 2) System.err.println("OOOOOPS! Window size is null!!"); 
				  ...
				  break;
			  // Added by Jeongbong Seo
			  case TELOPT_XDISPLAY: // X window. No
				  if(debug > 2) System.err.println("X Display");
				  reply = WONT;
				  break;
				  
			  case TELOPT_TSPEED:
				  if(debug > 2) System.err.println("Termial Speed");
				  reply = WONT;
				  break;
				  
			  case TELOPT_NENV:
				  if(debug > 2) System.err.println("New EnvOpt");
				  reply = WONT;
				  break;
				  
			  case TELOPT_REMOTE_FLOW_CNT:
				  if(debug > 2) System.err.println("Remote Flow Control");
				  reply = WONT;
				  break;		  
		  ...
		  case STATE_IACSB:
			  if(debug > 2) System.err.println("SubOption : "+b+"\n");
			  switch (b) {
			  case IAC:
		  ...
		  case STATE_IACSBDATA:
			  if (debug > 2) System.err.println("ToDo"+b+" ");
			  switch (b) {
			  case IAC:
		  ...
		  case STATE_IACSBDATAIAC:
			  if (debug > 2) System.err.println("What?"+b+" ");
			  switch (b) {
			  case IAC:
				  neg_state = STATE_IACSBDATA;
				  byte[] xsb = new byte[sbbuf.length+1];
				  ...
				  break;
			  case SE:
				  if (debug > 2) System.err.println("SuboptionEnd");
